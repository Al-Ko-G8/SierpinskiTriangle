...
export default function Page() {
  ...
  const size = 600
  const svgHeight = (Math.sqrt(3) / 2) * size
  ...
  const centerY = (currentView.y1 + currentView.y2) /2 // + currentView.y3) / 3

  // Calculate the viewBox based on the current view and zoom level
  const viewBoxWidth = (currentView.x3 - currentView.x1) / zoomLevel
  const viewBoxHeight = (currentView.y1 - currentView.y2) / zoomLevel
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 gap-8">
      ...
      <div className="border rounded-lg p-4 bg-white relative">
        <svg ref={svgRef} width={size} height={svgHeight} viewBox={viewBox} preserveAspectRatio="xMidYMid meet">
          <SierpinskiRecursive
            x1={0}
            y1={1000}
            x2={500}
